...
public class PorterStemmer {
     ...
     * PorterStemmer s = new PorterStemmer(); try { FileInputStream in = new
     * FileInputStream("c:\\test.txt");
     *
     * try { while (true) { int ch = in.read(); if (Character.isLetter((char)
     * ch)) { int j = 0; while (true) { ch = Character.toLowerCase((char) ch);
     * w[j] = (char) ch; if (j < 500) { j++; } ch = in.read(); if
     * (!Character.isLetter((char) ch)) { for (int c = 0; c < j; c++) {
     * s.add(w[c]); }
     *
     *
     * s.stem(); String u = s.toString();
     *
    ...
    public static void main(String[] args) {
        ...
        System.out.println(stemmer.stem("jhgjhgjgjgs"));
    }
}
